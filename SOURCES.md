...
The exterior is an original schematic surface, not factory CAD and not a downloaded scan. Its envelope follows published Model 3 overall dimensions: wheelbase 2,875 mm, length about 4,694 mm, width about 1,849 mm, height about 1,443 mm ([Tesla Model 3, Wikipedia dimension table](https://en.wikipedia.org/wiki/Tesla_Model_3)). Wheel openings, the glasshouse, and the lamps sit in that envelope. The surface itself is not a factory panel. Anything that is not established below is labeled **schematic, not to scale** in the interface.
